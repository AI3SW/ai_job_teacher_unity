...
namespace UnityDecoupledBehavior
{
    public class SoundManager : MonoBehaviour
    {
        ...
        public void TTS()
        {
            StartCoroutine(playfromOnline());
        }

        IEnumerator playfromOnline()
        {
            string jobname = AppManager.Singleton.getJobData(AppManager.Singleton.job_Catalogue.currentSelectedJobId).name;
            string url = AppManager.Singleton.getServerAddress();
            using (UnityWebRequest www = UnityWebRequestMultimedia.GetAudioClip(url +"/audio/" + jobname, AudioType.MPEG))
            {
                ...
                else
                {
                    ...
                    SFX_SoundSrc.clip = myClip;
                    SFX_SoundSrc.Play();
                    Debug.Log("playfromOnline");
                }
            }
