...
public class WebcamController : MonoBehaviour
{
	...
	public void generateWebcamTextureString(ImageFormat format)
    {
		...
		float MaskToCameraTextureHeight = 0;
		float snapshotHeight = 0;
		float snapshotWidth = 0;
		float maskToPotraitRatio = MaskWidth / PotraitWidth;

#if (UNITY_ANDROID || UNITY_IOS) && !UNITY_EDITOR
		heightOffset = cameraTexture.height;
		heightOffset *= ((1f - maskToPotraitRatio) / 2f);

		MaskToCameraTextureHeight = cameraTexture.width / MaskHeight;
		snapshotHeight = cameraTexture.width * maskToPotraitRatio;
		snapshotWidth = MaskHeight * MaskToCameraTextureHeight;
#else
		widthOffset = cameraTexture.width;
		widthOffset *= ((1f - maskToPotraitRatio) / 2f);

		MaskToCameraTextureHeight = cameraTexture.height / MaskHeight;
